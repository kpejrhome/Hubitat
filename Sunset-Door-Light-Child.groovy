...
def childSetup(){ 
    dynamicPage(name: "childSetup", title: "Sunset Door Light - Child", nextPage: null, install: true, uninstall: true, refreshInterval: 0) {
        ...
        section("Device Section"){
            ...
            input "doubleClickVariable", "capability.variable", title: "Double Click Variable to Double Timer?", multiple: false, required: false
            ...
            input name: "doubleOffTimerMinutes", type: "number", title: "How many minutes to wait till turning off if Switch is Double Clicked?", defaultValue: 0, required: true
...
def initialize(){
    ...
    subscribe(settings.targetSwitch, "doubleTapped.1", buttonOnHandler)
}
...
def buttonOnHandler( evt ){
      ...
      for(device in settings.doubleClickVariable){
        log.info "Variable Set to true: ${device.getLabel()}"
        device.setVariable(true)
      }
}
...
def turnLightsOn(){
    ...
    if(settings.offTimerMinutes > 0){
        def offSeconds = settings.offTimerMinutes * 60
        logDebug "Off Seconds ${offSeconds}"
        
        runIn(offSeconds, "turnLightsOff")
    }
}
...
def turnLightsOff(){
      def doubleSet = false
       ...
       for(device in settings.doubleClickVariable){
        log.info "Variable Set to false: ${device.getLabel()}"
         doubleSet = device.currentValue("variable")
         device.setVariable(false)
      }
    
    log.info "doubleSet = ${doubleSet}"
      ...
      if(doubleSet == "false") {
         ...
         def offSeconds = settings.doubleOffTimerMinutes * 60
        log.info "Restarting Timer ${offSeconds} : seconds"
          runIn(offSeconds, "turnLightsOff")
    }   
}
        section("Device Section"){
